...
def trackStellarProperties(simulation_directory, short_name, index, prior_indices, property_list, float32=True):
  '''
  ...
  pointers = m12.Pointer.get_pointers(species_name_from='star', species_names_to='star')
  ...
  for prop in property_list:
    ...
    arr = np.nan*np.empty(len(indices)) if len(p.shape) == 1 else np.nan*np.empty([len(indices), p.shape[1]])
  ...
  ut_io.file_hdf5(f'temporary_stellar_tracking/{short_name}_{index}', dict_or_array_to_write=output, verbose=True)

def runParallelStellarPropertyTracking(simulation_directory, indices, property_list, snapshots=np.arange(1, 600, 1), proc_number=
2, float32=True):
  '''
  ...
  ut_io.file_hdf5(f'temporary_stellar_tracking/{short_name}_600', dict_or_array_to_write=output, verbose=True)
  
  # Free up some memory for the parallel computing
  del output
  del z0

  args = [[simulation_directory, short_name, snapshot, indices, property_list] for snapshot in snapshots]
